<?php
/**
 * A model for managing Glantz modules.
 * 
 * @package GlantzCore
 */
class CMModules extends CObject {
   ...
   * Properties
   */
  private $glantzCoreModules = array('CGlantz', 'CDatabase', 'CRequest', 'CViewContainer', 'CSession', 'CObject');
  private $glantzCMFModules = array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CHTMLPurifier');
 ...
 /**
  ...
  private function GetDetailsOfModule($module) {
    ...
    if(class_exists($module)) {
      ...
      $details['isGlantzCore']   = in_array($rc->name, $this->glantzCoreModules);
      $details['isGlantzCMF']    = in_array($rc->name, $this->glantzCMFModules);
